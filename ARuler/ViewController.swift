...
class ViewController: UIViewController {
   ...
   func setup() {
      ...
      infoLabel.text = "Initializing the Wolrld"
      targetImageView.image = UIImage(named: "WhiteTarget")
   }
   ...
   func showActionSheet() {
      ...
      let meterAction = UIAlertAction(title: "Meter", style: .default) { (action) in
         self.unit = .meter
      }
      let centimeterAction = UIAlertAction(title: "Centimeter", style: .default) { (action) in
         self.unit = .centimeter
      }
      let inchAction = UIAlertAction(title: "Inch", style: .default) { (action) in
         self.unit = .inch
      }
...
extension ViewController: ARSCNViewDelegate {
   ...
   func session(_ session: ARSession, didFailWithError error: Error) {
      ...
      infoLabel.text = "An Error Occured"
   }
